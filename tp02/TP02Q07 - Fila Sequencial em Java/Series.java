...
class FilaCircular {
  ...
  public void inserir(Series serie) throws Exception {
    ...
    int fim = ultimo;
    while (ini != fim) {
      count = count + series[ini].getNtemporadas();
      ini = (ini + 1) % series.length;
    }
    count = (int) Math.round((double) count / added);
    MyIO.println(count);
  }
...
class Series {
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (i = 0; i < totalInseridos; i++) {
      fila.inserir(ler("../series/" + allIn[i]));
    }
    i = 0;
    ...
    verifyProcess(fila, allIn2, totalaProcessar);
  }

  public static void verifyProcess(FilaCircular fila, String[] allIn2, int totalaProcessar) throws Exception {
    int count = 0;
    for (int i = 0; i < totalaProcessar; i++) {
      String[] tratament = allIn2[i].split(" ");
      if (tratament[0].charAt(0) == 'I') {
        fila.inserir(ler("../series/" + tratament[1]));
  ...
  public static String pegaAtributoComRestricoesN(String a) {
    ...
    int iC = 0;
    int iL = 0;
    while (iL < str.length && !find) {
      if (onlyNumbers(str[iL].charAt(0))) {
        while (onlyNumbers(str[iL].charAt(iC))) {
          atributo += str[iL].charAt(iC++);
          find = true;
        }
        iL++;
